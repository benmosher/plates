...
// Helper: encode an ExportedWorkout using the legacy v1 (named-key JSON) format
// so we can verify backward compatibility without depending on internal functions.
async function encodeLegacyFormat(exported: {
  name: string;
  groups: Array<{
    movements: Array<{ name: string; maxName: string | null; sets: Array<{ reps: number; count: number; weight: number }> }>;
    restSeconds?: number;
    notes?: string | null;
  }>;
}): Promise<string> {
  ...
  const bin = Array.from(result, (b) => String.fromCharCode(b)).join("");
  return btoa(bin).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
}

describe("backward compatibility", () => {
  test("decodes legacy v1 (named-key JSON) format", async () => {
    const legacy = {
      name: "Legacy Day",
      groups: [
        {
          movements: [
            { name: "Squat", maxName: "Squat", sets: [{ reps: 5, count: 3, weight: 80 }] },
          ],
          restSeconds: 180,
        },
      ],
    };
    const encoded = await encodeLegacyFormat(legacy);
    const decoded = await decodeWorkout(encoded);
    expect(decoded.name).toBe("Legacy Day");
    expect(decoded.groups[0].movements[0].maxName).toBe("Squat");
    expect(decoded.groups[0].restSeconds).toBe(180);
    expect(decoded.groups[0].movements[0].sets).toEqual([{ reps: 5, count: 3, weight: 80 }]);
  });
});

describe("v2 packed format efficiency", () => {
  test("encoded string is shorter than legacy v1 format", async () => {
    const legacyExported = {
      name: workout.name,
      groups: workout.groups.map((g) => ({
        movements: g.movements.map((m) => ({
          name: m.name,
          maxName: m.maxId === 1 ? "Squat" : m.maxId === 2 ? "Bench" : null,
          sets: m.sets,
        })),
        ...(g.restSeconds != null ? { restSeconds: g.restSeconds } : {}),
      })),
    };
    const legacyEncoded = await encodeLegacyFormat(legacyExported);
    const v2Encoded = await exportWorkout(workout, maxes);
    expect(v2Encoded.length).toBeLessThan(legacyEncoded.length);
  });
});
